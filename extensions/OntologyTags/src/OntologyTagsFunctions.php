...
class OntologyTagsFunctions {
	...
	public static function getOntologyTags( $pwId ) {
		...
		//$tags = $xml->xpath("/gpml:Pathway/gpml:Biopax/bp:openControlledVocabulary/bp:TERM/text()");
		$entry = $xml->Biopax[0];
		$namespaces = $entry->getNameSpaces( true );
		$bpNS = $entry->children( $namespaces['bp'] );
		$total = count( $bpNS->openControlledVocabulary );
...
class_alias( "WikiPathways\\OntologyTags\\OntologyTagsFunctions", "OntologyTagsFunctions" );
