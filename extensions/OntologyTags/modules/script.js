...
var ontologies = mw.config.get( "ontologies" );
var oTagsCount = new Array();
var opath = mw.config.get("opath");
var optagloggedIn = mw.config.get("otagloggedIn");;


for(var i=0;i<Object.keys(ontologies).length;i++)
{
	document.getElementById('ontologyTags').innerHTML += "<div id='" + Object.values(ontologies)[i][0] + "'><b>" + Object.values(ontologies)[i][0] + "</b> : </div>";
	oTagsCount[Object.values(ontologies)[i][0]] = 0;
}
fetchTags();
...
function toggleOntologyDisplay()
{
	for(var i=0;i<Object.keys(ontologies).length;i++)
	{
		var ontologyName = Object.values(ontologies)[i][0];
