...
function fetchPathwayList( $imageMode ) {
	...
	switch ( $_GET['filter'] ) {
		...
		case 'Edited' : {
		   ...
		   } else { $resultTable .= "<td width='10%'>&nbsp;</td><td width='35%' align='left'>$display</td><td width='10%'>&nbsp;</td></tr>";
		   }
					$count++;
				}
			}
...
function fetch_tree() {
	...
	$xml = simplexml_load_string( ontologycache::fetchCache( "tree", ontologyfunctions::getBioPortalURL( 'tree', [ "ontologyId" => $ontology_id, "conceptId" => $concept_id ] ) ) );
