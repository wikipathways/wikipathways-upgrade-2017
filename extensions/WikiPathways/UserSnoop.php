	...
	function execute( $par ) {
		...
		if ( $this->targetUser ) {
			if ( $wgRequest->getVal( 'userid' ) && $wgRequest->getVal( 'useraction' ) ) {
				...
				$this->uid = $dbr->selectField( 'user', 'user_id', [ 'user_name' => $this->targetUser ], __METHOD__ );
			}
	...
	function getPageHeader() {
		...
		if ( $this->targetUser != "" ) {
			...
			$reg = $dbr->selectField( 'user',
				...
				[ 'user_name' => $this->targetUser ], __METHOD__ );
...
class UserSnoopPager extends AlphabeticPager {
	...
	function __construct( $uid=0 ) {
		if ( $uid > 0 ) {
			$this->uid = $uid;
			$dbr = wfGetDB( DB_SLAVE );
			$this->targetUser = $dbr->selectField( 'user', 'user_name', [ 'user_id' => $this->uid ], __METHOD__ );
		} else {
			global $wgRequest;
